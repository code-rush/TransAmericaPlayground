					...
					if (Physics.Raycast (ray, out hit)) {
						if (Input.GetMouseButtonDown (0)) {
							if (hit.collider.gameObject.tag == "dLine") {
								...
								if (playersCollections [0].Contains (p1) || playersCollections [0].Contains (p2)) {
									if (playersCollections [0].Contains (p1)) {
										playersCollections [0].Add (p2);
									} else {
										playersCollections [0].Add (p1);
									}
								...
								foreach (Vector3 p in playersCollections[0]) {
